...
export default function RecordingDetailPage() {
  ...
  const handleDelete = async () => {
    try {
      ...
      router.push(`/projects/${projectSlug}/library`);
    } catch {
      setDeleting(false);
      setDeleteOpen(false);
    }
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Button
            ...
            size="sm"
            onClick={() =>
              router.push(`/projects/${projectSlug}/tests/new-from-recording?recordingId=${recordingId}`)
            }
          >
              ...
              {linkedTests.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      router.push(
                        `/projects/${projectSlug}/tests/new-from-recording?recordingId=${recordingId}`
                      )
                    }
                  >
                    <TestTube2 className="h-4 w-4 mr-1.5" />
                ...
                <div className="space-y-3">
                  {linkedTests.map((test) => (
                    <Link
                      key={test.id}
                      href={`/projects/${projectSlug}/suites/${test.suiteId}/tests/${test.id}`}
