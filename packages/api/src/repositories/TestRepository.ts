...
export interface TestCreateData {
  ...
  recordingId?: string | null; // FK to recordings library
 ...
 */
function toTestSummary(test: Partial<Test> & { suiteName?: string | null }): TestSummary {
  return {
    id: test.id!,
    userId: test.userId!,
    projectId: test.projectId!,
...
export class TestRepository {
  ...
  async findBySlug(
    ...
    const conditions = includeDeleted
      ...
      : and(eq(tests.projectId, projectId), eq(tests.slug, slug), isNull(tests.deletedAt));
  ...
  async findMany(
    ...
    // Build conditions
    const conditions = [eq(tests.userId, filters.userId), eq(tests.projectId, filters.projectId)];
   ...
   */
  async findAllBySuite(userId: string, suiteId: string): Promise<SafeTest[]> {
    const result = await this.db
      .select()
      .from(tests)
      .where(and(eq(tests.userId, userId), eq(tests.suiteId, suiteId), isNull(tests.deletedAt)))
   ...
   */
  async findAllByProject(userId: string, projectId: string): Promise<SafeTest[]> {
    const result = await this.db
      .select()
      .from(tests)
      .where(and(eq(tests.userId, userId), eq(tests.projectId, projectId), isNull(tests.deletedAt)))
      .orderBy(asc(tests.displayOrder), asc(tests.name));
   ...
   * Update a test
   */
  async update(id: string, userId: string, data: TestUpdateData): Promise<SafeTest | null> {
    const updateData: Partial<NewTest> = {};

    if (data.name !== undefined) updateData.name = data.name;
    if (data.description !== undefined) updateData.description = data.description;
    if (data.suiteId !== undefined) updateData.suiteId = data.suiteId;
    if (data.recordingId !== undefined) updateData.recordingId = data.recordingId;
    ...
    if (data.slug !== undefined) {
      ...
      const existing = await this.findByIdAndUser(id, userId);
      if (!existing) return null;
      updateData.slug = await this.ensureUniqueSlug(existing.projectId, data.slug, id);
    ...
    const result = await this.db
      .update(tests)
      .set(updateData)
      .where(and(eq(tests.id, id), eq(tests.userId, userId), isNull(tests.deletedAt)))
      .returning();
   ...
   */
  async updateLastRun(id: string, data: TestLastRunUpdate): Promise<void> {
    await this.db
      .update(tests)
      .set({
   ...
   */
  async moveToSuite(testIds: string[], userId: string, targetSuiteId: string): Promise<number> {
    if (testIds.length === 0) return 0;

    const result = await this.db
      .update(tests)
      .set({ suiteId: targetSuiteId })
      .where(and(inArray(tests.id, testIds), eq(tests.userId, userId), isNull(tests.deletedAt)))
      .returning();
   ...
   */
  async reorder(userId: string, suiteId: string, orderedIds: string[]): Promise<void> {
  ...
  async countByProject(userId: string, projectId: string): Promise<number> {
    const result = await this.db
      ...
      .from(tests)
      .where(
        and(eq(tests.userId, userId), eq(tests.projectId, projectId), isNull(tests.deletedAt))
      );
   ...
   */
  async isSlugAvailable(projectId: string, slug: string, excludeId?: string): Promise<boolean> {
    const conditions = [
   ...
   */
  async ensureUniqueSlug(projectId: string, baseSlug: string, excludeId?: string): Promise<string> {
    let slug = baseSlug;
    let counter = 1;
   ...
   */
  async isNameAvailable(suiteId: string, name: string, excludeId?: string): Promise<boolean> {
    const conditions = [
      eq(tests.suiteId, suiteId),
